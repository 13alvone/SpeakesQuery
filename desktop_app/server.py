    ...
    for raw in uniq:
        ...
        try:
            ...
            if files:
                ...
                # request threads. A 2026-05-18 production incident
                # (InvalidInputException on near-simultaneous requests)
                # forced this pattern across all IMMUTABLE reads.
                con = duckdb.connect(database=":memory:")
                try:
                    con.execute("PRAGMA threads=1")
                    try:
                        # union_by_name=true protects against IMMUTABLE-tree
                        # schema additivity (ag_picks and friends all
