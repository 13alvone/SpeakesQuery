...
**What it is.** Scripts may declare `trust_level: "unrestricted"`: plain `compile()`, full `__builtins__`, no import filter (`executor.py::_build_unrestricted_globals`). 21 of the 131 shipped scripts use it, for scipy/sklearn/rapidfuzz-grade computation the sandbox cannot express. The `_pro` suffix on the script name and its output subdirectory keeps the tier visible everywhere the script appears.
