...
tools/                Operational utilities shipped *in* the Docker image (unlike tests/)
  ...
  curator_takeout_import.py  Phase 6 / Bet 5 slice 1 (2026-05-16): one-shot Google Takeout (YouTube) bootstrap → `indexes/IMMUTABLE/curator_takeout/{subscriptions,playlists_metadata,playlist_videos,watch_history}/*.parquet`. Stdlib-only HTML/CSV parsing - no BeautifulSoup. Handles missing channel fields (private/deleted videos) + unknown TZ abbrevs (epoch falls back to now-UTC, raw `tz_abbrev` preserved for forensics). Re-runnable; each invocation produces a new `<epoch>_<uuid>.system4.system4.parquet`. See `docs/lang/21_curator_speaktube.md`.
  ...
  curator_topic_snapshot_refresh.py  One-shot/CLI topic-vector snapshot refresh (engine also schedules it weekly when `curator_topic_snapshot_refresh_enabled`)
...
docs/lang/
  ...
  21_curator_speaktube.md         Curator ↔ speaktube contract (Phase 6 / Bet 5 slice 1): four REST endpoints, IMMUTABLE storage, Google Takeout bootstrap, hourly telemetry pull
  22_speaktube_handoff.md         speaktube build handoff (Bet 5 slice 1 companion): SpeakesQuery deliverables vs speaktube deliverables, MVP checklist
  23_speaktube_post_launch_deltas.md  Consolidated speaktube handoff for slices 4-12 (2026-05-17): per-slice consumer updates + migration notes
```
...
- Change the response shape of any speaktube-contract endpoint (`GET /api/playlist/today`, `GET /api/dignity/today`, `POST /api/reflections`, `POST /api/growth_dial`) without coordinating with the speaktube renderer. Phase 6 / Bet 5 slice 1 (2026-05-16) pins the exact JSON shape both sides agreed to in `docs/lang/21_curator_speaktube.md`. The two non-obvious contract semantics: (a) `GET /api/playlist/today` MUST return 404 (not 200 + empty `items[]`) when no composition has happened yet - the renderer distinguishes "no curator wired up" from "you literally have a quiet day"; (b) `GET /api/dignity/today` MUST return 200 + `dignity_pct: null` (not 404) when no plays today, so the player's footer renders an unambiguous "offline" rather than alternating between 200 and 404 every minute. Adding NEW optional fields to the response is fine - the renderer is forward-compatible per the contract. Removing or renaming an existing field breaks the renderer silently. Pinned by `tests/test_curator_speaktube_slice1.py` (4 endpoint contract tests). Same pattern will apply to any future external-system-facing endpoint where SpeakesQuery's API surface has a non-SpeakesQuery consumer with its own deploy cadence.
- Drop a column from any curator_* IMMUTABLE-bound schema (`curator_telemetry`, `curator_reflections`, `curator_playlist`, `curator_topic_snapshots`) once shipped. Phase 6 / Bet 5 slice 1 (2026-05-16) extended the OEB additivity-only rule to the curator's forever-data streams: the user's decade-horizon viewing telemetry, their written reflections, the historical record of what the curator suggested each day, AND (slice 3, 2026-05-16) the topic-evolution timeline ("what was the user into 6 months ago vs today?"). Removing a column breaks every historical SPQL query touching it - and unlike a typical bug, the failure is silent until the operator runs an old query 18 months later. Pinned by `tests/test_curator_speaktube_slice1.py::test_curator_*_schema_is_additive_only` + `tests/test_curator_topic_vectors_slice3.py::test_curator_topic_snapshots_schema_is_additive_only` (frozen column snapshots). Same pattern as `tests/test_oeb_wave2.py::test_immutable_schema_is_additive_only` - generalises to any future IMMUTABLE schema (Phase 7 broker journal, Phase 5 trade ledger, etc.).
- Bypass the `apply_topic_scoring` AG flag and apply the dispatcher's slice-3 topic-scoring hook globally. The hook (`alert_groups/dispatcher.py::_maybe_apply_topic_scoring`) is OPT-IN per AG because (a) AGs without a `title` column would crash the cosine-scoring step, and (b) non-curator AGs (OEB, GMRB, etc.) score by their own bespoke columns. Opt-in via AG YAML; the dispatcher hook short-circuits cleanly when the flag is absent or false. If a future AG type needs scoring against a DIFFERENT snapshot kind (Phase 5 broker-style snapshot, Phase 7 reader-profile snapshot), add a new flag (`apply_broker_scoring`, etc.) - don't reuse `apply_topic_scoring` for cross-domain hooks. Pinned by `tests/test_curator_topic_vectors_slice3.py::TestDispatcherTopicScoringHook` (4 behaviours: flag-off, df-None, no-snapshot, happy-path).
- Add a new AG with `output_kind != "picks"` without ALSO adding (a) the routing branch in `alert_groups/dispatcher.py` AFTER the existing `_extract_and_log_picks` call, (b) the parser/writer/extractor methods (mirror `_parse_playlist_block` / `_log_playlist_items` / `_extract_and_log_playlist`), and (c) a frozen-source-string drift guard in the slice's test class asserting `'output_kind == "X"'` appears in the dispatcher source. The curator playlist composer (Phase 6 / Bet 5 slice 2, 2026-05-16) is the first AG with `output_kind != "picks"` - future AGs using different write targets (Phase 5 broker order log, Phase 7 mobile push log, etc.) follow the same three-place wiring. Pinned by `tests/test_curator_composer_slice2.py::TestDryRunMoneyLeakCanary::test_output_kind_routing_pinned_in_source`. Per-AG `dry_run: true` on a non-picks AG MUST short-circuit the LLM call (money-leak canary) - pinned by the same test class.
- Use the module-level `duckdb.sql(...)` helper to read curator/IMMUTABLE parquets from a Flask request handler, APScheduler job, or any code that can run on a worker thread. The `duckdb.sql()` helper shares a SINGLE module-level default connection across all callers, and DuckDB connections are NOT thread-safe for concurrent execution. Two near-simultaneous callers (e.g. `GET /api/preferences/keywords` hitting `read_active_curator_keyword_pool` + `GET /api/search`) can leave the global connection in a bad state, producing the misleading `InvalidInputException: Attempting to execute an unsuccessful or closed pending query result` on the second caller's `.fetchall()`/`.df()`. Caught 2026-05-18 on prod after the user typed in the Discover search box (UI fires keyword-prefs save + search back-to-back). Always use a per-call connection: `con = duckdb.connect(database=":memory:"); con.execute("PRAGMA threads=1"); ...; con.close()` in a try/finally. Same pattern `/api/dignity/today` uses (which was the only curator endpoint NOT broken in this incident). Pinned by `tests/test_curator_immutable_read_robustness.py::test_no_module_level_duckdb_sql_for_immutable_reads` (drift guard across all three files that read IMMUTABLE data).
- Read a multi-file IMMUTABLE-tree parquet glob (any `read_parquet([files...])` or `read_parquet('<glob>')` against `indexes/IMMUTABLE/curator_*` / `ag_picks` / future IMMUTABLE schemas) WITHOUT `union_by_name=true`. Two unrelated production 500s on 2026-05-18 (`/api/dignity/today` and `/api/search`) traced to the SAME root cause: DuckDB picks the FIRST-sorted file's schema for the whole glob, and IMMUTABLE trees naturally have schema-heterogeneous files. Two distinct failure modes: (a) the `curator_telemetry_pull` ingestion script writes a parquet PER FIRE even when zero events were fetched - pyarrow infers all-None string columns as the Null logical type, and any `WHERE event_date = ?` / `chosen_by IN (...)` against a Null-typed column fails with `Conversion Error: Unimplemented type for cast (VARCHAR -> "NULL")`; (b) additive schema growth means older parquets lack newer columns (e.g. pre-slice-4 `curator_candidates` parquets lack `thumbnail_url`), and a `COALESCE(thumbnail_url, '')` against a missing column trips the misleading binder error `Column "thumbnail_url" referenced that exists in the SELECT clause - but this column cannot be referenced before it is defined`. The fix is `union_by_name=true` - it promotes Null-typed columns to VARCHAR (using the wider type across files) AND fills missing columns with NULL so binder resolution always succeeds. Applies to EVERY multi-file read in `desktop_app/server.py` (curator endpoints + the generic `_probe_index_freshness` since OEB IMMUTABLE schemas grow too), `alert_groups/dispatcher.py` (thin-history detection), and `functionality/log_writer.py::get_active_keyword_pool` (active-keyword-pool cutoff + read). Pinned by `tests/test_curator_immutable_read_robustness.py` - 12 tests covering both reproducers + a drift guard scanning every pinned call site + a scanner that catches any future copy-paste of an unprotected `read_parquet(` near a `curator_` / `IMMUTABLE` token in `desktop_app/server.py`. When introducing a new IMMUTABLE-tree read site, add it to `_PINNED_READ_SITES` AND include `union_by_name=true` in the SQL.
- Add a new curator candidate source (Archive.org / PBS / Vimeo / etc.) that emits to `indexes/IMMUTABLE/curator_candidates/` without matching the canonical 14-column shape established by `curator_youtube_rss_pull.json` (slice 1.5 + slice 4) and extended by `curator_archive_org_pull.json` (slice 7). The candidate row schema (`_epoch`, `discovered_at_epoch`, `source`, `video_external_id`, `video_url`, `title`, `channel_id`, `channel_name`, `channel_url`, `published_iso`, `description`, `duration_seconds`, `thumbnail_url`, `raw_blob`) is the contract between the ingestion layer (multiple sources, growing over time) and the slice-2 composer (single consumer). `thumbnail_url` was added 2026-05-17 (slice 4) so the composer can thread image URLs through to `/api/playlist/today`. Each new source bumps its own `source` string (`archive_org` is Tier 1 since slice 7; future Tier 2: `dailymotion` / `vimeo` / `tubi` / `pluto_tv`) AND adds a parallel `| append` branch in `default_saved_searches/curator_scored_candidates_today.yaml` so the composer reads the entire candidate pool with one SPQL query. Pinned by `tests/test_curator_slice_1_5_candidates.py::test_curator_candidate_schema_is_additive_only_in_script` + `tests/test_curator_slice3b_topic_search.py::test_emits_same_canonical_columns_as_slice_1_5` (the second now compares ALL THREE sources, not just slice 1.5 vs 3b - extend the slice-list when adding source #4). If a future source genuinely needs a NEW column, add it to the frozen set, update both tests, and document the back-fill semantics for older sources (they emit NULL for the new column).
