...
alert_groups/         Multi-search Claude API dispatch
  ...
  dispatcher.py           Orchestrates: serialize → Claude (or the local LLM router when the AG sets `model_id`, Slice A 2026-06-23) → email → log; emits failure-alert email on error; `delivery_mode: prompt_only` forks to email-the-prompt path (no Claude call, $0 cost) for budget-friendly runs. Graceful-completion overhaul (2026-08-04): graduated retry with backoff for local LLM calls (transient-only; HTTP 4xx config errors fail fast), half-open circuit breaker (cooldown skips are clean + auto-probe after `alert_group_circuit_breaker_cooldown_hours`, success closes the breaker), salvage prompt email on terminal LLM failure so feeder data is never lost, 24h rate-limit window shrunk by a grace margin (completion-stamped rows were dropping every other daily dispatch), and optional per-AG `email_digest_model_id` that distills the raw analyst output into a BLUF-first email via a $0 local model (raw response still feeds pick extraction + the .md attachment). Pinned by tests/test_ag_graceful_2026_08_04.py
